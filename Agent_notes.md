...
Trying to standardize the calibration so using datasets from huggingface. Currently grabbed a random split of [UCSC-VLAA/GPT-Image-Edit-1.5M](https://ucsc-vlaa.github.io/GPT-Image-Edit/) (omni) and [ma-xu/fine-t2i](https://arxiv.org/abs/2602.09439) (text/image pairs).
Flux2 Klein is distilled to 4 steps.
I don't know how many points we need so this may be overkill.
Currently we have:
- ~1000 curated t2i prompts from fine-t2i but we only grabbed part 1 for PoC.